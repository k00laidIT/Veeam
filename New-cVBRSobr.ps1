<#
.Synopsis
For Veeam Backup & Replication v12 this script will create the desired number of buckets with
    a given prefix and then add them into a SOBR.
...
An AWS access key must be configured in Veeam Backup & Replication with a description matching
    the name of the AWS profile configured in the previous step.
...
New-cVBRSOBR -NamePrefix 'testset1' -VBRSrv 'localhost' -SvcPoint 'https://us-central-1a.object.ilandcloud.com' -RegionId 'us-central-1a' -AWSProfile 'myprofile' -NumRepos '5' -IMM -IMMDays '30'
#>
...
function New-cVBRSOBR {
  ...
  param (
    ...
    [Parameter(ParameterSetName = 'IMM', Mandatory = $true)]
    [Switch] $IMM,
